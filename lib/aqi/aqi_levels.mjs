...
export const AQI_ALGORITHM_VERSION = "aqilevels_hourly_v2";
...
export const DAQI_PM_ROLLING24H_WINDOW_HOURS = 24;
export const DAQI_PM_ROLLING24H_REQUIRED_HOURS = 18;
...
export function expandPmHelperRowsToHourlyGrid(helperRows, options = {}) {
  const rangeStartHour = parseIsoHour(options.rangeStartUtc);
  const rangeEndHour = parseIsoHour(options.rangeEndUtc);
  const rangeStartMs = rangeStartHour ? Date.parse(rangeStartHour) : null;
  const rangeEndMs = rangeEndHour ? Date.parse(rangeEndHour) : null;
  const byTimeseries = new Map();
  for (const row of Array.isArray(helperRows) ? helperRows : []) {
  ...
  for (const rows of byTimeseries.values()) {
    ...
    if (!template || !["pm25", "pm10"].includes(template.pollutant_code)) {
      expanded.push(...rows);
      continue;
    }
    const rowsByHour = new Map(rows.map((row) => [row.timestamp_hour_utc, row]));
    let startMs = Date.parse(rows[0].timestamp_hour_utc);
    let endExclusiveMs = Date.parse(rows[rows.length - 1].timestamp_hour_utc) + HOUR_MS;
    if (Number.isFinite(rangeStartMs)) startMs = Math.min(startMs, rangeStartMs);
    if (Number.isFinite(rangeEndMs) && rangeEndMs > startMs) endExclusiveMs = Math.max(endExclusiveMs, rangeEndMs);
    for (let cursor = startMs; cursor < endExclusiveMs; cursor += HOUR_MS) {
      const hourIso = new Date(cursor).toISOString();
      const existing = rowsByHour.get(hourIso);
      if (existing) {
        existing._synthetic_hour = false;
        expanded.push(existing);
        continue;
      }
      expanded.push({
        timeseries_id: template.timeseries_id,
        station_id: template.station_id,
        connector_id: template.connector_id,
        pollutant_code: template.pollutant_code,
        timestamp_hour_utc: hourIso,
        ...
        _synthetic_hour: true,
      });
    }
  }
  return sortHelperRows(expanded);
}

export function pivotNarrowRowsToHelperRows(narrowRows, options = {}) {
  const byKey = new Map();
  ...
  const helperRows = expandPmHelperRowsToHourlyGrid(sortHelperRows(Array.from(byKey.values())), options);
  ...
  for (const rows of byTimeseries.values()) {
    ...
    for (let currentIndex = 0; currentIndex < rows.length; currentIndex += 1) {
      ...
      for (let previousIndex = currentIndex; previousIndex >= 0; previousIndex -= 1) {
        ...
        if (diffHours >= DAQI_PM_ROLLING24H_WINDOW_HOURS) break;
...
export function helperRowToNormalizedAqiV1Row(row, options = {}) {
  ...
  const daqiRequiredCount = daqiInputAveragingCode === "rolling_24h_mean"
    ? DAQI_PM_ROLLING24H_REQUIRED_HOURS
    : 1;
...
export function helperRowsToNormalizedAqiV1Rows(helperRows, options = {}) {
  return (Array.isArray(helperRows) ? helperRows : [])
    .map((helperRow) => ({
      helperRow,
      row: helperRowToNormalizedAqiV1Row(helperRow, options),
    }))
    .filter(({ helperRow, row }) =>
      row &&
      row.connector_id &&
      row.station_id &&
      row.timeseries_id &&
      row.timestamp_hour_utc &&
      (!helperRow._synthetic_hour || row.daqi_calculation_status === "ok")
    )
    .map(({ row }) => row)
    .sort((left, right) => {
      if (left.timeseries_id !== right.timeseries_id) return left.timeseries_id - right.timeseries_id;
...
export function buildAqilevelHistoryRowsForDayFromSourceObservations(rows, dayUtc, options = {}) {
  const helperRows = pivotNarrowRowsToHelperRows(sourceObservationsToNarrowRows(rows), {
    rangeStartUtc: utcDayStartIso(dayUtc),
    rangeEndUtc: utcDayEndIso(dayUtc),
  });
...
export function buildAqilevelHistoryRowsByDayFromSourceObservations({ rows, fromDayUtc, toDayUtc, computedAtUtc = null }) {
  const helperRows = pivotNarrowRowsToHelperRows(sourceObservationsToNarrowRows(rows), {
    rangeStartUtc: utcDayStartIso(fromDayUtc),
    rangeEndUtc: utcDayEndIso(toDayUtc),
  });
  const output = [];
