...
- Backend, database, scheduler, R2 and Integrity date handling remains UTC-only. Do not introduce `Europe/London`, BST, DST or browser-local date conversion into this work.
...
Use a stable UTC `logical_run_date` for the scheduled daily profile.

- Use UTC exclusively for the logical run date, historical target-day rotation, missed-run detection and SQLite state identity.
- Prefer the existing scheduled date supplied to daily-task-health reporting only when it is already the scheduler's UTC scheduled date.
- Otherwise derive it from the current UTC date, equivalent to `utc_now().date()`.
- Do not convert through `Europe/London`, BST, DST, the host machine timezone or browser-local time.
- Do not derive it from the latest R2 date.
- A retry for the same scheduled UTC day must reuse the same logical date.
...
- UTC logical run date: `2026-07-17`
...
The UTC logical run date determines the historical target day number or numbers. The latest R2 date and the UK local calendar date do not determine this rotation.
...
This mapping means each target day number from 1 to 31 is allocated exactly once during every UTC logical calendar month.

The allocation applies to the UTC logical run month only. When applying a target day number to an earlier historical month:
...
1. apply every target day number allocated to the UTC logical run date;
...
- the historical dates allocated for the current UTC logical run date;
- any historical target allocation carried forward from missed UTC logical dates, as described below.
...
A suitable logical model is one row per environment and UTC logical run date, for example:
...
Use a unique key on `(env_name, logical_run_date)` and idempotent upsert behaviour. Rerunning the same UTC logical day must update the same state record rather than silently creating duplicate scheduling state.
...
Use the SQLite state to detect UTC logical daily dates after the last completed daily profile that have no completed state.

- On the first run after this feature is deployed, seed from the current UTC logical date. Do not retrospectively generate catch-up work for all dates before deployment.
- For later missed or failed UTC logical dates, calculate the historical target day number allocation that would have applied on each missed date and add those historical target dates to the next daily explicit selection.
...
- `selection_mode`
- `logical_run_date`
- `logical_run_timezone`, fixed to `UTC`
- `latest_r2_observations_day`
...
2. Use `grep`, as required by `AGENTS.md`, to identify:
   ...
   - every timezone or local-date conversion in the Integrity launcher, scheduler and daily-task-health path;
...
3. Keep all helper inputs, calculations and persisted scheduling dates UTC-only.
4. Preserve manual contiguous-window behaviour.
5. Preserve weekly and monthly CLI behaviour.
6. Decide the smallest compatibility changes required for functions that currently accept only `from_day` and `to_day`.
7. Ensure no daily call can accidentally expand the historical selection into a continuous multi-year range.
8. Confirm how the existing local Dropbox v2 observations root is resolved and reuse that resolution rather than introducing a second path contract.
...
2. Derive the scheduled logical date from the UTC scheduled date or current UTC date only.
3. Implement the seven-day recent selection.
4. Implement the historical target-day allocation table.
5. Discover represented historical months from actual v2 observations day directories.
6. Produce the sorted, deduplicated explicit target-date set with reasons.
7. Keep manual, weekly and monthly semantics unchanged.
8. Fail safely when no R2 observations day can be discovered.
9. Remove or retire daily-only use of `INGESTDB_RETENTION_DAYS` without removing retention behaviour needed by other systems.
10. Keep legacy summary bounds only as compatibility metadata, not processing scope.

Deliverable: daily selection is data-driven from the local R2 mirror, UTC-only and produces an explicit non-contiguous date set.
...
6. Implement post-deployment-only catch-up behaviour for missed UTC logical dates.
...
3. Configure and document the scheduler timezone as UTC.
4. Preserve the existing environment, source filter, history version, check-only or repair mode, limits, lock, logging and daily-task-health settings.
5. Keep weekly and monthly CLI profiles available.
6. After the new daily operation has been proven in TEST, disable redundant scheduled weekly or monthly invocations if they exist. Do not delete the profiles from the CLI as part of this refactor.
7. If the active launchd plist or scheduler configuration lives outside the repository, do not edit it automatically. Provide the exact operator command or file change required after the code is deployed.
8. Do not run `launchctl`, deploy workflows or other external operations unless the user explicitly grants Level 4 permission.

Deliverable: code and exact operator instructions for a single UTC daily TEST run, without silently changing operational mode.
...
- a normal UTC date such as 17 July;
- proof that the logical date remains the UTC date around a BST/local-midnight boundary;
...
3. install or reload any repo-managed launcher or external launchd schedule change with UTC scheduling;
...
1. confirm `logical_run_date` is the intended UTC date and no `Europe/London`, BST or DST conversion occurred;
2. confirm the latest discovered R2 observations day matches the latest day directory in the local R2 mirror;
3. confirm the recent selection contains exactly seven consecutive dates ending on that day;
4. confirm the historical selection contains the allocated target date for each represented earlier month where that date is calendar-valid;
5. confirm the current anchor month is not separately added as historical scope;
6. confirm the report lists explicit selected dates and reasons;
7. confirm no intervening days between the oldest historical target and the recent window were processed unless explicitly selected;
8. confirm OpenAQ, Sensor.Community and SOS adapters respect the selected dates;
9. confirm SOS annual files are not redundantly downloaded or parsed for each selected date where reuse is possible;
10. confirm SQLite has one idempotent state row for the UTC logical date and that the existing `integrity_runs` record remains intact;
11. confirm daily-task-health receives the intended status and UTC selection summary;
12. confirm existing v2 detection, report and repair-plan behaviour remains unchanged for the selected days.
...
- The TEST Integrity schedule runs once per day using UTC scheduling.
- `logical_run_date`, historical target rotation, catch-up detection and SQLite identity are UTC-only.
- No `Europe/London`, BST, DST, host-local or browser-local conversion is used in backend Integrity date selection.
...
- Historical target dates are allocated by the UTC logical run date using the stated 28, 29, 30 and 31-day mapping.
