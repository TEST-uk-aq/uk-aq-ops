# Codex Prompt: Move UK AQ Fixed Scheduling to Cloudflare Schedulers
...
```text
Cloudflare = fixed schedule scheduler / safety checks
...
### Move fixed schedules to Cloudflare scheduler workers
...
The Cloudflare ingest scheduler should check things like:
...
A scheduler decision should roughly be:
...
The scheduler should check things like:
...
Add two Cloudflare Worker schedulers if that fits the repo structure:
...
If there is already a good Cloudflare scheduler pattern in the repo, reuse it instead of inventing a separate style.
...
Use one ingest scheduler and one ops scheduler unless the existing repo style strongly suggests otherwise.
...
However, still implement scheduler-side guards:
...
- Remove GCP Scheduler jobs until the Cloudflare scheduler is tested.
...
- Inventory note written at `plans/cloudflare scheduler - Cflare latest-snapshot builder/ukaq-cloudflare-scheduler-phase-1-inventory.md`
- Current scheduler map captured for ingest, ops, OpenAQ safety, latest snapshot, and backup jobs
- Viability check passed, with one caveat: the ops scheduler will need a minimal state extension for jobs not currently covered by `daily_task_health`
- Auth direction captured: Cloudflare should use a shared dispatch header later rather than trying to reuse the current GCP Scheduler OIDC path directly

### Phase 2: Add Cloudflare schedulers in dry-run/log-only mode

The schedulers should be able to run and log decisions without triggering Cloud Run.
...
- Added phase-2 dry-run Cloudflare scheduler workers, with the ingest scheduler moved to the ingest repo and the ops scheduler kept in the ops repo
...
- Confirmed the phase-2 scheduler package passes `node --check` and `node --test`
...
2. Cloudflare ingest scheduler Worker.
3. Cloudflare ops scheduler Worker.
...
Other ingest fixed schedules:
  Cloudflare ingest scheduler -> existing Cloud Run ingest services

Ops fixed schedules:
  Cloudflare ops scheduler -> existing Cloud Run ops services
