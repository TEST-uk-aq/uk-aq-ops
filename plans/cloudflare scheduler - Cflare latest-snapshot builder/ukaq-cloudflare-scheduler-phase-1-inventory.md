...
The fixed-schedule Cloudflare scheduler model fits the current repo layout, but one implementation caveat needs to be carried into later phases:

- `daily_task_health` already covers the daily ops jobs that are clearly instrumented.
- `uk-aq-db-size-logger`, `uk-aq-aqilevels-retention-service`, and `uk-aq-timeseries-aqi-hourly` do not all have matching daily-task-health entries today, so the ops scheduler will need either a minimal state extension or a service-specific due-check source for those jobs.
...
- `/Users/mikehinford/Dropbox/Projects/UK-AQ Website & Network/TEST UK-AQ GH Repos/TEST-uk-aq-ingest/system_docs/uk_aq_cloudflare_scheduler_ingest.md`
...
| --- | --- | --- | --- | --- |
| `uk-aq-blondon-communities-trigger` | `uk-aq-blondon-communities-ingest` | GCP Cloud Scheduler every 15 min | Cloudflare ingest scheduler | `uk_aq_ingest_runs` |
| `uk-aq-blondon-nodes-trigger` | `uk-aq-blondon-nodes-ingest` | GCP Cloud Scheduler every 15 min | Cloudflare ingest scheduler | `uk_aq_ingest_runs` |
| `uk-aq-sos-trigger` | `uk-aq-sos-ingest` | GCP Cloud Scheduler every 15 min | Cloudflare ingest scheduler | `uk_aq_ingest_runs` |
...
| `uk-aq-db-size-logger-hourly` | `uk-aq-db-size-logger` | GCP Cloud Scheduler hourly | Cloudflare ops scheduler | Daily-task-health style run history or a small task-health extension |
| `uk-aq-prune-daily` | `uk-aq-prune-daily` | GCP Cloud Scheduler daily | Cloudflare ops scheduler | `daily_task_health` |
| `uk-aq-observs-partition-maintenance-daily` | `uk-aq-observs-partition-maintenance-service` | GCP Cloud Scheduler daily | Cloudflare ops scheduler | `daily_task_health` |
| `uk-aq-aqilevels-retention-daily` | `uk-aq-aqilevels-retention-service` | GCP Cloud Scheduler daily | Cloudflare ops scheduler | Needs either a `daily_task_health` extension or a minimal dedicated due-check source |
| `uk-aq-timeseries-aqi-hourly-trigger` plus reconcile variants | `uk-aq-timeseries-aqi-hourly` | Multiple GCP Cloud Scheduler jobs | Cloudflare ops scheduler, but only after job-mode mapping is designed | Service-specific state or `uk_aq_ingest_runs` / task-history lookup |
