...
Deno.test("refresh metrics sum counts and retain maximum lag", () => {
  assertEquals(aggregateRefreshMetrics([
    {
      source_rows: 10,
      rows_upserted: 8,
...
Deno.test("failed chunk error exposes chunk metadata", () => {
  ...
  const error = new DeepRefreshChunkError(full, chunk, 2, 4, "statement timeout");
  ...
  if (!error.message.includes("helper upsert RPC failed for reconcile_deep chunk 2/4")) {
