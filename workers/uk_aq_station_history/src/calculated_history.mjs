...
function calculateLogicalAqi(observationRows, request, continuity, outputStartMs, outputEndMs) {
  ...
  return helperRowsToNormalizedAqiV1Rows(
    ...
    const endpoint = Date.parse(row.timestamp_hour_utc);
    const physical = memberAt(continuity, endpoint);
...
export async function buildCalculatedHistory({ request, continuity, env, outputStartMs, outputEndMs, ingestRows = [], ingestComplete = false, ingestFetchCount = 0, guideline = null } = {}) {
  ...
  const observationGaps = [...selection.gaps, ...missingRanges(expected, observationPresent)];
  const aqiGaps = request.includeAqi ? [...selection.gaps, ...missingRanges(expected, aqiPresent)] : [];
  ...
  const observationsComplete = sourceComplete && observationGaps.length === 0;
  ...
  const aqiComplete = request.includeAqi && sourceComplete && aqiGaps.length === 0 && aqiStatusesComplete;
  ...
  return {
    ...
    } : { enabled: false, calculation_source: null, rows: [], response_complete: false, has_gap: false, gap_ranges: [] },
    source: {
