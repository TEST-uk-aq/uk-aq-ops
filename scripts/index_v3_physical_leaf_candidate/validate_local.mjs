...
function parse(argv) {
  const options = { alignedRoot: "", physicalRoot: "", leafRoot: "" };
  ...
  return Object.fromEntries(
    Object.entries(options).map(([key, value]) => [key, path.resolve(value)]),
  );
}
...
function localSource({ alignedRoot, physicalRoot, leafRoot }) {
  const read = (key) => {
    const root = key.startsWith(`${LEAF_PREFIX}/`)
      ? leafRoot
      : key.startsWith(`${PHYSICAL_PREFIX}/`)
        ? physicalRoot
        : alignedRoot;
...
async function main() {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    scoped_manifest_count: structural.manifestCount,
    leaf_count: structural.leafCount,
    leaf_plan_objects: structural.plan.objects.length,
    parquet_objects_created: structural.plan.parquet_objects.length,
    physical_index_total_bytes: physicalTotalIndexBytes,
    leaf_index_total_bytes: structural.totalIndexBytes,
