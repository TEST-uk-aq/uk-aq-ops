...
const PREFIX = "history/_prototype/observation-history/timeseries-aligned-v2/candidate=physical-leaf-index-v1/cap_rows=1024";
...
export function classifyExistingPhysicalLeafCandidateObject({ head, intent }) {
...
async function main() {
  ...
  if (
    ...
    plan.index_root !== INDEX_ROOT || plan.aligned_row_cap !== 1024 ||
    plan.physical_leaf_candidate_version !== "physical-leaf-index-v1" ||
    plan.reuses_aligned_parquet !== true || plan.parquet_objects?.length !== 0
  ) throw new Error("publication plan is not the exact TEST physical-leaf candidate plan");
  ...
  const intents = plan.objects.map((entry) => {
    ...
    ) throw new Error(`unsafe or duplicate physical-leaf-candidate publication key: ${entry.key}`);
    seen.add(entry.key);
  ...
  for (const intent of intents) {
    ...
    if (existing?.exists !== false) {
      try {
        const unchanged = classifyExistingPhysicalLeafCandidateObject({ head: existing, intent });
