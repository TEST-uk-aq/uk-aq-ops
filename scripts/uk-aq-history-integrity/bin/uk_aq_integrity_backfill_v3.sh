...
usage() {
  cat <<'USAGE'
Usage:
  uk_aq_integrity_backfill_v3.sh [options]
...
Optional:
  The history generation is fixed to v3; --history-version is rejected.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --history-version|--history-version=*)
      echo "ERROR: --history-version is not accepted by the fixed-v3 SOS-light bridge." >&2
      exit 2
      ;;
    --dry-run)
      DRY_RUN=true
...
INCOMING_SOS_SOURCE_ACQUISITION_MODE="$(trim "${UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_MODE:-}")"
INCOMING_SOS_SOURCE_ACQUISITION_ROOT="$(trim "${UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_ROOT:-}")"
INCOMING_SOS_SOURCE_ACQUISITION_RUN_ID="$(trim "${UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_RUN_ID:-}")"
INCOMING_CORE_SNAPSHOT_IDENTITY_JSON="$(trim "${UK_AQ_INTEGRITY_CORE_SNAPSHOT_IDENTITY_JSON:-}")"
INCOMING_CORE_SNAPSHOT_IDENTITY_FILE="$(trim "${UK_AQ_INTEGRITY_CORE_SNAPSHOT_IDENTITY_FILE:-}")"
INCOMING_CORE_SNAPSHOT_DROPBOX_ROOT="$(trim "${UK_AQ_INTEGRITY_CORE_SNAPSHOT_DROPBOX_ROOT:-}")"
...
INCOMING_TARGET_WRITER_GIT_SHA="$(trim "${UK_AQ_INTEGRITY_TARGET_WRITER_GIT_SHA:-}")"
...
if ! [[ "${INCOMING_TARGET_WRITER_GIT_SHA}" =~ ^[0-9a-f]{40}$ ]]; then
  echo "ERROR: UK_AQ_INTEGRITY_TARGET_WRITER_GIT_SHA must be a full lower-case Git SHA." >&2
  exit 4
fi
...
if [[ -n "${INCOMING_SOS_SOURCE_ACQUISITION_MODE}" ]]; then
  export UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_MODE="${INCOMING_SOS_SOURCE_ACQUISITION_MODE}"
fi
if [[ -n "${INCOMING_SOS_SOURCE_ACQUISITION_ROOT}" ]]; then
  export UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_ROOT="${INCOMING_SOS_SOURCE_ACQUISITION_ROOT}"
fi
if [[ -n "${INCOMING_SOS_SOURCE_ACQUISITION_RUN_ID}" ]]; then
  export UK_AQ_BACKFILL_SOS_SOURCE_ACQUISITION_RUN_ID="${INCOMING_SOS_SOURCE_ACQUISITION_RUN_ID}"
fi
export UK_AQ_INTEGRITY_CORE_SNAPSHOT_IDENTITY_JSON="${INCOMING_CORE_SNAPSHOT_IDENTITY_JSON}"
export UK_AQ_INTEGRITY_CORE_SNAPSHOT_IDENTITY_FILE="${INCOMING_CORE_SNAPSHOT_IDENTITY_FILE}"
export UK_AQ_INTEGRITY_CORE_SNAPSHOT_DROPBOX_ROOT="${INCOMING_CORE_SNAPSHOT_DROPBOX_ROOT}"
...
export UK_AQ_INTEGRITY_TARGET_WRITER_GIT_SHA="${INCOMING_TARGET_WRITER_GIT_SHA}"
...
INTEGRITY_WRAPPER="${SCRIPT_DIR}/uk_aq_integrity_backfill_v3.sh"
...
export UK_AQ_R2_HISTORY_VERSION="v3"
export UK_AQ_R2_HISTORY_INDEX_VERSION="v3"
...
echo "core_snapshot_identity_file: ${UK_AQ_INTEGRITY_CORE_SNAPSHOT_IDENTITY_FILE}"
echo "core_snapshot_stage: ${UK_AQ_INTEGRITY_CORE_SNAPSHOT_STAGE}"
