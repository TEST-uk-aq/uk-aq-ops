...
export UK_AQ_R2_HISTORY_VERSION UK_AQ_R2_HISTORY_INDEX_VERSION UK_AQ_R2_HISTORY_INTEGRITY_VERSION
...
PY_ENTRY="${UK_AQ_HISTORY_INTEGRITY_ROOT}/bin/uk-aq-history-integrity-sos-light-v3.py"
...
export UK_AQ_CORE_SNAPSHOT_DROPBOX_ROOT="${R2_ROOT}/history/v3/core"
export UK_AQ_HISTORY_INTEGRITY_BACKFILL_WRAPPER="${REPO_ROOT}/scripts/uk-aq-history-integrity/bin/uk_aq_integrity_backfill_v3.sh"
...
for ((i = 0; i < ${#REMAINING_ARGS[@]}; i++)); do
  case "${REMAINING_ARGS[i]}" in
    ...
    --history-version)
      error "--history-version is not accepted by the fixed-v3 SOS-light runner"
      ((i += 1))
      ;;
    --history-version=*) error "--history-version is not accepted by the fixed-v3 SOS-light runner" ;;
  esac
done
