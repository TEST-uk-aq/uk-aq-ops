...
const SOURCE_INDEX_ROOT = `${BASE_PREFIX}/cap_rows=2048/observations_timeseries`;
...
async function inspectFile({ alignedRoot, alignedPlanByKey, descriptor, requiredSegments }) {
  ...
  for (const segment of requiredSegments) {
    ...
    if (rowCount !== segment.row_count || segment.row_group_row_start !== 0) {
...
async function main() {
  ...
  if (!sourceChildren.length || !sourceManifests.length) throw new Error("no cap_rows=2048 aligned indexes found");
