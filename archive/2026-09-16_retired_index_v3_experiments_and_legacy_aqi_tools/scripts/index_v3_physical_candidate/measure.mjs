...
const ALIGNED_HOST = /-v3-aligned-candidate\.[a-z0-9-]*test[a-z0-9-]*\.workers\.dev$/i;
const PHYSICAL_HOST = /-v3-physical-candidate\.[a-z0-9-]*test[a-z0-9-]*\.workers\.dev$/i;

export function buildPhysicalCandidateMeasurementMatrix() {
  return [
    { name: "aurn_ts218_24h", connector_id: 1, timeseries_id: 218, start_utc: "2026-08-20T00:00:00.000Z", end_utc: "2026-08-21T00:00:00.000Z" },
...
function parse(argv) {
  const options = {
    alignedEndpoint: process.env.UK_AQ_V3_ALIGNED_CANDIDATE_URL || "",
    physicalEndpoint: process.env.UK_AQ_V3_PHYSICAL_CANDIDATE_URL || "",
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === "--aligned-endpoint") options.alignedEndpoint = next();
    else if (argument === "--physical-endpoint") options.physicalEndpoint = next();
...
function requestUrl(base, item, variant) {
  ...
  if (variant === "aligned_2048") url.searchParams.set("aligned_row_cap", "2048");
  return url;
}
...
async function attempt({ base, item, variant, number, secret, timeoutMs }) {
  ...
  try {
    const response = await fetch(requestUrl(base, item, variant), {
...
async function main() {
  ...
  const matrix = buildPhysicalCandidateMeasurementMatrix();
  if (options.dryRun) {
    process.stdout.write(`${JSON.stringify({ repeat: options.repeat, variants: ["aligned_2048", "physical_index_v1"], cases: matrix }, null, 2)}\n`);
    return;
  }
  const endpoints = {
    aligned_2048: endpoint(options.alignedEndpoint, ALIGNED_HOST, "aligned endpoint"),
    physical_index_v1: endpoint(options.physicalEndpoint, PHYSICAL_HOST, "physical endpoint"),
  };
  ...
  const directory = path.resolve(options.outputDir || path.join("tmp", "index_v3_physical_measurements", stamp.replaceAll(/[-:]/g, "")));
  ...
  for (const item of matrix) {
    for (let number = 1; number <= options.repeat; number += 1) {
      for (const variant of ["aligned_2048", "physical_index_v1"]) {
        sequence += 1;
