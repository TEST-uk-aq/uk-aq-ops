...
def _read_parquet_partition_stats(files: Iterable[Path]) -> tuple[dict[str, Any] | None, str | None]:
    ...
    try:
        ...
        try:
            null_timeseries_count = total_row_count - sum(counts.values())
        except (TypeError, ValueError):
...
def _append_parent_aggregate_gaps(
    ...
    *,
    ...
    # For Phase 2c.1: focus on row_count, source_row_count, file_count as core required fields
    required_fields = ("row_count", "source_row_count", "file_count")
...
def _append_actual_parquet_gaps(
    ...
    *,
    ...
    # Validate required pollutant manifest fields
    # For Phase 2c.1: focus on row_count, source_row_count, file_count as core required fields
    # total_bytes, min_timeseries_id, max_timeseries_id validation is optional for compatibility
    required_pollutant_fields = ("row_count", "source_row_count", "file_count")
    for field in required_pollutant_fields:
        actual = _int_field(payload, field)
        if actual is None:
            # Missing or invalid type - schema mismatch
            gaps.append(gap_fn(
                f"data_manifest_{field}_schema_mismatch",
                day_utc=day_utc,
                connector_id=connector_id,
                pollutant_code=pollutant_code,
                expected_path=manifest_rel,
    ...
    # Validate min/max timeseries_id - can be None or integers if present
    for field in ("min_timeseries_id", "max_timeseries_id"):
        if field in payload:
            value = payload.get(field)
            if value is not None and not isinstance(value, int):
                # Present but not None or an integer - schema mismatch
                gaps.append(gap_fn(
                    f"data_manifest_{field}_schema_mismatch",
                    day_utc=day_utc,
                    connector_id=connector_id,
                    pollutant_code=pollutant_code,
                    expected_path=manifest_rel,
                    related_paths=[f"field={field} invalid_type"],
                ))
            ...
            for pollutant_dir in pollutant_dirs:
                ...
                if not manifest_path.is_file():
                    ...
                    if isinstance(payload, dict):
                        ...
                        # Handle zero vs missing row_count properly for pollutant manifests
                        # Missing or invalid type are handled by required field validation above
                        # Genuine zero is allowed by writer, so no additional gap needed for row_count == 0
