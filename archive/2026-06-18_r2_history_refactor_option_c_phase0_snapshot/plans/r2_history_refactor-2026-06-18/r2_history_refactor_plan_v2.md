...
1. Record current deployed URLs and route ownership.
2. Confirm whether `/api/aq/timeseries` is served by cache proxy Worker, Supabase Edge Function, or another layer.
3. Capture baseline responses for known test cases.
4. Confirm v2 index root files and coverage.
5. Confirm v2 coverage start date and whether it is expected.
6. Confirm observations/AQI one-day lag is expected or explain it.
7. Capture current Station Snapshot v2 local rows behaviour for `/api/station-snapshot-v2/rows`.
8. Confirm whether Station Snapshot v2 chart-history probes forward `connector_id`, `pollutant`, and `debug=1`.
9. Confirm whether deployed `workers/uk_aq_dashboard_online_api_worker/src/lib/station_snapshot_v2.ts` has the same metadata gaps as the local server.
...
1. No code changes.
2. Written report of route ownership and current behaviour.
3. Known failing/successful cases documented.
4. Current v2 index layout confirmed as intentional.
5. Station Snapshot v2 baseline documents local HTML request params, local server chart-history params, direct R2 API params, and deployed dashboard Worker params.
