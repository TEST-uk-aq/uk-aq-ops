...
- R2 history writers and index builders use `CFLARE_R2_BUCKET` directly.
  Test and LIVE have separate Cloudflare/R2 accounts, so no deploy-env bucket
  selector is needed.
...
```bash
env | grep -E 'LIVE|PROD|R2_BUCKET|CFLARE_R2_BUCKET|UK_AQ_DROPBOX_ROOT|UKAQ_ENV_NAME|UK_AQ_ENV_NAME' | sort
```
