...
9. treat checkpoint arrival as necessary but not sufficient evidence that the local Dropbox client has finished materialising the refreshed baseline. Before the month is marked complete or a later month starts, run a bounded local-materialisation gate tied to the same exact backup run. That gate MUST authenticate the locally readable files newly copied or replaced by that backup which can be consumed by the next SOS-light baseline, using the exact backup report plus the generation-specific checkpoint/state evidence and canonical manifest/hash identities. For observations this includes every observation day copied by the exact backup and its complete local dependency graph of day/connector/pollutant manifests and Parquet objects. For core and timeseries-binding backup representations, any units copied or replaced by the exact backup and needed by SOS-light MUST likewise be locally readable and match their checkpoint/manifest/pack identities. A locally present checkpoint root alone MUST NOT satisfy this gate;
10. only then start the next month, whose own SOS-light Step 0 still performs the normal backup-ordering and live-R2 root-hash equality gates.
