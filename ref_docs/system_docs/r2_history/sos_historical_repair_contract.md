...
3. after a successful month, including the final month in the requested batch, dispatch the normal environment-matched **R2 History Dropbox Backup** workflow with `observation_parquet_copy_mode=reuse_matching`; omission of the backup option remains `full` for other callers and is not the contracted monthly mode;
...
8. after the cloud backup succeeds, obtain its backup report evidence and require the report to confirm effective `observation_parquet_copy_mode=reuse_matching`. The report's Parquet reuse/copy counters and avoided-byte evidence are audit information; they do not replace the normal successful-complete/root evidence. If the exact report does not confirm the requested effective mode, stop the monthly batch;
9. wait until the local Dropbox checkpoint used by Integrity reflects that same successful backup generation. At minimum, the local checkpoint's processed observations-root identity MUST match the successful backup report's processed observations-root identity;
10. treat checkpoint arrival as necessary but not sufficient evidence that the local Dropbox client has finished materialising the refreshed baseline. Before the month is marked complete or a later month starts, run a bounded local-materialisation gate tied to the same exact backup run. That gate MUST authenticate the locally readable files required by the next SOS-light baseline, using the exact backup report plus the generation-specific checkpoint/state evidence and canonical manifest/hash identities. For files copied or replaced by the exact backup, normal current-run local materialisation evidence applies. For observation Parquet bodies explicitly reported as reused under `reuse_matching`, the verifier MAY carry forward the preceding accepted local baseline evidence only when the current manifest still references the exact same key, byte size and canonical SHA-256 and the local file remains present/readable at the expected path. Reused Parquet is not required to gain a new Dropbox modification time or new `copied_at` evidence. Any mismatch, missing local file, ambiguous prior authority or contradictory report/manifest identity fails the local gate. For observations the resulting accepted local day must still have a complete readable dependency graph of day/connector/pollutant manifests and Parquet objects. For core and timeseries-binding backup representations, any units copied or replaced by the exact backup and needed by SOS-light MUST likewise be locally readable and match their checkpoint/manifest/pack identities. A locally present checkpoint root alone MUST NOT satisfy this gate;
11. only then start the next month, whose own SOS-light Step 0 still performs the normal backup-ordering and live-R2 root-hash equality gates.
...
A fixed cooldown or arbitrary sleep is not a substitute for this evidence. The former 300-second monthly cooldown is not part of the contract and SHOULD be removed. Waiting is driven by the exact backup run, the matching local checkpoint identity and the authenticated local materialisation of the refreshed files. The local-materialisation verifier MUST be read-only with respect to R2 and Dropbox cloud state; it proves local availability and identity only.
...
The monthly summary MUST retain enough operator evidence to diagnose the chain, including the month, caller correlation identifier, exact GitHub Actions backup run ID and URL, terminal conclusion, effective observation Parquet copy mode, Parquet reuse/copy counts and avoided-byte totals from the backup report where available, backup report processed observations-root identity and the matching local checkpoint identity.
